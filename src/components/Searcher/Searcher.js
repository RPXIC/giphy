...
const Searcher = (({ history, initialRating = 'g', initialLanguage = 'en' }) => {
    ...
    return (
        <form className="searcher" onSubmit={ e => {
            e.preventDefault()
            const query = e.target.query.value
            if (query.trim() === '') return
            history.push(`/search/${query}/${rating}/${language}`)
        }}>
            <input onChange={handleChangeTimes} className="searcher__input" type="text" name="query" placeholder="search" />
            <button className="searcher__button"><i className="fas fa-search"></i></button>
            {/* {times} */}

            <div>
                <select onChange={handleChangeRating} className="searcher__select" value={rating}>
                    <option disabled>Rating content</option>
                    {RATINGS.map((rating, index) => <option key={rating} value={rating}>{RATINGS_TEXT[index]}</option>)}
                </select>
                <select onChange={handleChangeLanguage} className="searcher__select" value={language}>
                    <option disabled>Language</option>
                    {LANGUAGES.map((language, index) => <option key={language} value={language}>{LANGUAGES_TEXT[index]}</option>)}
                </select>
            </div> 
        </form>
    )
